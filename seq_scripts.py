    ...
    if epoch == 6667:
        ...
        with open(f"{work_dir}/{mode}_visual.txt", "w") as file:
            for item in range(len(gls_ref)):
                ...
                file.write('CNN :  ' + gls_hyp1[item] + "   " + str(
                    wer_list(hypotheses=[gls_hyp1[item]], references=[gls_ref[item]])['wer']) + "\n")
                file.write('LSTM:  ' + gls_hyp2[item] + "   " + str(
                    wer_list(hypotheses=[gls_hyp2[item]], references=[gls_ref[item]])['wer']) + "\n\n")
